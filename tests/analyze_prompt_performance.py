...
def analyze_performance(
    ...
    # Print findings
    console.print("\n[bold cyan]═══ PROMPT PERFORMANCE ANALYSIS ═══[/bold cyan]\n")

    # Short responses table
    if short_responses:
        console.print(f"[bold red]⚠ SHORT RESPONSES (< 50% expected length)[/bold red]")
    ...
    if low_keyword_match:
        console.print(f"[bold yellow]⚠ LOW KEYWORD OVERLAP (< 10%)[/bold yellow]")
    ...
    # Slow responses
    if slow_responses:
        console.print(f"[bold blue]🐌 SLOW RESPONSES (> 30s)[/bold blue]")
    ...
    # Overall statistics
    console.print("[bold green]📊 OVERALL STATISTICS[/bold green]\n")

    # Check if comparisons is empty to avoid ZeroDivisionError
    if not comparisons:
        console.print("[yellow]⚠ No comparisons found to analyze[/yellow]")
        avg_length_ratio = 0
        avg_keyword_sim = 0
        avg_time = 0
    else:
        ...
        console.print(f"• Average length ratio: [bold]{avg_length_ratio:.2f}[/bold] (target: 1.0)")
        console.print(f"• Average keyword similarity: [bold]{avg_keyword_sim:.3f}[/bold] (target: > 0.15)")
        console.print(f"• Average response time: [bold]{avg_time:.1f}s[/bold]")
    console.print()
    ...
    if avg_length_ratio < 0.7:
        console.print("❶ [yellow]Responses are significantly shorter than expected[/yellow]")
        console.print("  → Add explicit length guidance in system prompt")
        console.print("  → Use phrases like 'provide a comprehensive analysis of at least 3-4 paragraphs'")
        console.print("  → Consider adding 'Be thorough and detailed in your response'\n")

    if avg_keyword_sim < 0.15:
        console.print("❷ [yellow]Low keyword overlap suggests responses are missing expected content[/yellow]")
        console.print("  → Make expectations more explicit in prompts")
        console.print("  → Add specific philosophical frameworks to discuss (e.g., 'Compare utilitarian and deontological perspectives')")
        console.print("  → Include required concepts in the prompt itself\n")

    if len(short_responses) > 10:
        console.print("❸ [yellow]Many prompts producing short responses[/yellow]")
        console.print("  → Review system prompt for any brevity instructions")
        console.print("  → Add 'min_tokens' or similar constraints if API supports it")
        console.print("  → Test with higher temperature (0.7-0.8) for more expansive responses\n")
    ...
    for prompt_id, prompt_data in live_data.get("results", {}).items():
        ...
        for variant in prompt_data.get("variants", []):
            if 'variant_index' not in variant:
                continue  # Skip variants without an index key
            variant_id = f"{prompt_id}::variant{variant.get('variant_index', '<missing>')}"
